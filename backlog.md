...
| US05 | As a PMO, I want to visualize KPIs in dashboards so that developers and stakeholders can monitor code quality and application health | Technical dashboard implemented in Grafana for engineering metrics; Executive dashboard implemented in Power BI for high-level KPIs, trends, and rankings | Medium | Sprint 3 | Done |
| US06 | As a PMO, I want to automate the engineering metrics pipeline so that metrics are updated regularly without manual intervention | GitHub Actions workflow that automatically extracts metrics and updates datasets that feeds dashboards | High | Sprint 4 | In progress |
